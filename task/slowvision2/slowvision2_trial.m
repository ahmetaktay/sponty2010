function history = slowvision2_trial(params, history, nTrial, percentNoTarget, isTarget, intensity)
% function history = slowvision2_trial(params, history, nTrial, percentNoTarget, [isTarget, intensity])
    ...
    [thisCorrect, thisResponseTime, keyCode] = getResponse(isTarget, params.response.yes, params.response.no, endStimulusTime + jitterQ);
    startResponseTime = Screen('Flip', params.wPtr);
    %%% Return textsize to normal
    Screen('TextSize', params.wPtr, oldTextSize);
    
    % Get Response
    [thisCorrect, thisResponseTime, keyCode] = getResponse(isTarget, params.response.yes, params.response.no, startResponseTime + params.timing.responseTime, '--startvals', [thisResponseTime, keyCode]);
    ...
    % old stuff done here
    history.timing.jitterQ = [history.timing.jitterQ, jitterQ];
