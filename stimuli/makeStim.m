function [gratings annulus fixation]=makeStim(params)
    ...
    % Get stimuli settings
    stimSettings = params.stimSet;
    ...
    spatialFrequency = 1 / stimSettings.pixelsPerPeriod; % How many periods/cycles are there in a pixel?
    radiansPerPixel = spatialFrequency * (2 * pi); % = (periods per pixel) * (2 pi radians per period)
    
    % *** If the grating is clipped on the sides, increase stimulusRadius
    halfWidthOfGrid = stimSettings.radius;
    ...
    ic_radius = oc_radius - stimSettings.outerRadius; % radius of inner circle
    ...
    outerFixation = ones(stimSettings.outerFixationSize, stimSettings.outerFixationSize);
    innerFixation = zeros(stimSettings.innerFixationSize, stimSettings.innerFixationSize);
    diffFixation = stimSettings.outerFixationSize - stimSettings.innerFixationSize;
    if mod(diffFixation, 2) ~= 0
       error('Difference between outer and inner fixations is not divisible by 2')
    end
    
    % put it together
    target_cells = (diffFixation/2 + 1):(stimSettings.outerFixationSize - diffFixation/2);
